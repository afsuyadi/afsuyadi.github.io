...
The sonar dataset that is obtained from underwater submarine is used to detect dangerous mines in order to avoid it. The device can only tell the difference between mines and rocks. I used logistic regression to build a machine learning model from this dataset to predict future outcomes.
...
The sales team has obtained datas from a mall department store that provides information regarding the amount of money spent by a customer followed with their annual salary income. Based on these data, we can investigate what type of customers that exists in order to construct future marketing strategy.
...
[View code on Colab](https://colab.research.google.com/drive/1dnEwc_itKnR2VfIdNaLM6-GlnNBR1XaX#scrollTo=Tl_Obm0aQ_cU)
